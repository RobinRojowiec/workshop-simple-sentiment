...
if __name__ == '__main__':
    ...
    cf.add_prediction("positive", "negative")
    cf.add_prediction("negative", "negative")
    print(cf.get_error_context_for_class("negative"))

    cf = ConfusionMatrix(["positive","negative"], np.matrix([[39, 12],[7, 44]]))
    print(cf)
    print(cf.get_error_context_for_class("negative"))

    print(cf.accuracy("negative"))
    print(cf.precision("negative"))
    print(cf.recall("negative"))
    print(cf.f_measure("negative"))

    print(cf.get_error_context_for_class("positive"))

    print(cf.accuracy_average())
    print(cf.recall_average())
    print(cf.precision_average())
    print(cf.f_measure_average())